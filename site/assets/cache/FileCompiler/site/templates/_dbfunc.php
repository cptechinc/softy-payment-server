<?php
	...
	function logsend($type, $date, $data) {
		$sql = \ProcessWire\wire('database')->prepare("INSERT INTO sendlogs (sendtype, datetimestamp, data) VALUES (:sendtype, :date, :data)");
	...
	function write_approved_response($ordn, $transactionid, $authcode, $avscode) {
		...
		$sql = \ProcessWire\wire('database')->prepare("INSERT INTO authnet (ordernbr, rectype, trans_id, authcode, avs_msg, result, date, time) VALUES (:ordn, 'RES', :transactionid, :authcode, :avscode, 'APPROVED', :date, :time)");
	...
	function write_declined_response($ordn, $errorcode, $errormsg) {
		$date = date('Ymd'); $time = date('His');
		$sql = \ProcessWire\wire('database')->prepare("INSERT INTO authnet (ordernbr, rectype, error_code, error_msg, result, date, time) VALUES (:ordn, 'RES', :errorcode, :errormsg, 'DECLINED', :date, :time)");
	...
	function write_declined_avs($ordn, $errorcode, $errormsg, $avscode) {
		$date = date('Ymd'); $time = date('His');
		$sql = \ProcessWire\wire('database')->prepare("INSERT INTO authnet (ordernbr, rectype, error_code, error_msg, avs_msg, result, date, time) VALUES (:ordn, 'RES', :errorcode, :errormsg, :avscode, 'DECLINED', :date, :time)");
	...
	function delete_authorize_record($ordn) {
		$sql = \ProcessWire\wire('database')->prepare("DELETE FROM authnet WHERE ordernbr = :ordn and rectype = 'REQ'");
		$switching = array(':ordn' => $ordn); $withquotes = array(true);
		$sql->execute($switching);
		return returnsqlquery($sql->queryString, $switching, $withquotes);
	}
	...
	function writeauthnetrecord($request, $debug) {
		$query = preparerequest($request);
 		$sql = \ProcessWire\wire('database')->prepare("INSERT INTO authnet (".$query['columns'].") VALUES(".$query['preparedvalues'].")");
	...
	function delete_authorize_responserecord($ordn) {
		$sql = \ProcessWire\wire('database')->prepare("DELETE FROM authnet WHERE ordernbr = :ordn and rectype = 'RES'");
		$switching = array(':ordn' => $ordn); $withquotes = array(true);
		$sql->execute($switching);
		return returnsqlquery($sql->queryString, $switching, $withquotes);
	}